...
- **毛玻璃一级卡片统一规格（防不一致铁律）**：所有直接浮在背景上的**第一层毛玻璃卡片**（使用 `FrostedGlassCard` 组件或内联实现），必须统一用**同一套参数**；严禁各页各写不同的透明度/描边，避免"有的卡白、有的卡淡、有的卡带高光边"割裂。
  - **浅色卡面**：`Color(0x75FFFFFF)`（白色 46%，与背景拉开清晰但不过分白的差距）
  - **深色卡面**：`Color(0xB818202F)`
  - **描边**：`Color(0x24FFFFFF)`（白色 ~14%，极淡，仅一丝呼吸）
  ...
  - **记录的例外**：背单词页的**答题/听写卡**（聚焦输入、需要更安静、更贴背景以突出内容）用 `Color(0x42FFFFFF)`（白色 26%，更淡）。除该例外外，其余第一层毛玻璃卡片统一用上面的 `0x66FFFFFF`。
- **第一层卡片边缘禁高光铁律**：**边缘绝对不要打"累赘感的高光描边"**。
  - ❌ **严禁**：给卡片套一圈半透亮白描边（如 `Color(0x80FFFFFF)` / `white alpha 0.5+`）当"玻璃边缘高光"——会显得刻意、不够简洁。
  - ✅ **正确做法**：边缘靠**卡面与背景的柔和亮度差 + 一点点柔影**来定义；即使描边也必须极淡（`white alpha ≤ 0.15`，如 `Color(0x24FFFFFF)`），只留一丝"呼吸"的定义，绝不抢镜。
  - 判断标准：把卡片放大到边缘，应看不到一圈明显的"亮线/高光圈"；边缘是静静过渡的。
