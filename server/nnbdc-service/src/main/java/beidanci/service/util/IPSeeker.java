...
public class IPSeeker {
    ...
    private final HashMap<String, IPLocation> ipCache;
    // 随机文件访问类
    ...
    // 为提高效率而采用的临时变量
    private final IPLocation cachedLocation;
    ...
    public List<IPEntry> getIPEntriesDebug(String s) {
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                IPLocation location = getIPLocation(temp);
                // 判断是否这个地点里面包含了s子串，如果包含了，添加这个记录到List中，如果没有，继续
                if (location.country.contains(s) || location.area.contains(s)) {
                    IPEntry entry = new IPEntry();
                    entry.country = location.country;
                    entry.area = location.area;
                    // 得到起始IP
                    readIP(offset - 4, b4);
    ...
    private IPLocation getIPLocation(long offset) {
        try {
            ...
            if (b == AREA_FOLLOWED) {
                ...
                b = ipFile.readByte();
                if (b == NO_AREA) {
                    cachedLocation.country = readString(readLong3());
                    ipFile.seek(countryOffset + 4);
                } else
                    cachedLocation.country = readString(countryOffset);
                // 读取地区标志
                cachedLocation.area = readArea(ipFile.getFilePointer());
            } else {
                cachedLocation.country = readString(ipFile.getFilePointer() - 1);
                cachedLocation.area = readArea(ipFile.getFilePointer());
            }
    ...
    private IPLocation getIPLocation(int offset) {
        ...
        if (b == AREA_FOLLOWED) {
            ...
            b = mbb.get();
            if (b == NO_AREA) {
                cachedLocation.country = readString(readInt3());
                mbb.position(countryOffset + 4);
            } else
                cachedLocation.country = readString(countryOffset);
            // 读取地区标志
            cachedLocation.area = readArea(mbb.position());
        } else {
            cachedLocation.country = readString(mbb.position() - 1);
            cachedLocation.area = readArea(mbb.position());
        }
        return cachedLocation;
    }
