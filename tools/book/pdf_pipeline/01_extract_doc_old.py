...
def extract_from_old_doc(input_path, output_path):
    ole = olefile.OleFileIO(input_path)
    
    # WordDocument stream 包含实际的文本
    # 尝试读取不同的stream
    words = []
    
    # 先尝试直接读取 WordDocument
    if ole.exists('WordDocument'):
        word_stream = ole.openstream('WordDocument').read()
        
        # 尝试从 FIB (File Information Block) 中找到文本
        # 简单方法：提取可见的ASCII文本
    ...
    # 也尝试读取 1Table 或 0Table
    for table_stream in ['1Table', '0Table']:
        if ole.exists(table_stream):
            try:
                stream_data = ole.openstream(table_stream).read()
...
if __name__ == '__main__':
    extract_from_old_doc('/Volumes/ssd/ppdc/tools/book/小学/译林版/最新译林版小学英语三-六年级单词汇总.doc', '/Volumes/ssd/ppdc/tools/book/pdf_pipeline/yilin_raw.txt')
