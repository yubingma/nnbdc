...
class AppleAiRuntime implements AiRuntime {
    ...
    _channel.setMethodCallHandler((call) async {
      if (call.method == 'onPartialResult') {
        try {
          ...
          Global.logger.w('[AppleAiRuntime] 处理部分结果失败: $e');
        }
      }
      return null;
    });
  ...
  Future<bool> initialize() async {
    try {
      ...
      if (!supported) {
        Global.logger.w('[AppleAiRuntime] ⚠️ 当前设备架构暂不支持本地 AI 功能');
      ...
      if (capResult is Map) {
        ...
        Global.logger.i('Apple AI 能力: $_capability, 平台: ${capResult['platform']}, 内存: ${capResult['memoryGB']} GB');
      }
      ...
      if (loadResult is Map && loadResult['success'] == true) {
        _isModelLoaded = true;
        Global.logger.i('Apple AI 模型加载成功: $modelPath');
        return true;
      } else {
        Global.logger.w('Apple AI 模型加载失败');
        return false;
      }
    } catch (e, st) {
      Global.logger.e('Apple AI 初始化失败', error: e, stackTrace: st);
      return false;
    }
  }
  ...
  Future<AiResponse> runTask(AiRequest request) async {
    ...
    try {
      ...
      Global.logger.d('Apple AI 推理 prompt 长度: ${prompt.length}');
      ...
      Global.logger.e('Apple AI 推理异常', error: e, stackTrace: st);
  ...
  Future<void> dispose() async {
    try {
      ...
      Global.logger.i('Apple AI 模型已卸载');
    } catch (e) {
      Global.logger.w('Apple AI 模型卸载失败: $e');
    }
  }
}
