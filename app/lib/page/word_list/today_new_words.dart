...
class TodayNewWordsProvider implements WordsProvider {
  ...
  Future<int> getWordIndex(String spell) async {
    var result = await WordBo().getTodayNewWordOrder(spell, Global.getLoggedInUser()!.id);
    if (result.success) {
...
class TodayNewWordsBookMarkProvider implements BookMarkProvider {
  static const String bookMarkName = 'today_new_words_list';
...
Future<dynamic>? toTodayNewWordsListPage(bool showDelBtn) {
  return Get.toNamed('/word_list',
      arguments: WordListPageArgs(
          '今日新词', TodayNewWordsProvider(), true, showDelBtn, true, '掌握度', TodayNewWordsProgressProvider(), TodayNewWordsBookMarkProvider(), null));
}
