...
class MyDatabase extends _$MyDatabase {
  ...
  Future<void> _migrateStudyStepFromV1ToV2() async {
    await transaction(() async {
      // ========== 第一步：字段重命名（统一命名规范） ==========
      // 这些重命名操作需要先执行，因为后续的查询会使用新字段名
      // 注意：新字段名使用驼峰格式
      
      // 1.1 重命名 user_db_logs 表的 table_ 字段为 tblName
      try {
        await customStatement('''
          ALTER TABLE user_db_logs 
          RENAME COLUMN table_ TO tblName
        ''');
      } catch (e) {
        // 如果字段不存在或已经重命名，忽略错误
        // 可能的原因：字段已经迁移过，或者表结构已经是新版本
      }
      
      // 1.2 重命名 user_study_steps 表的 index 字段为 seq
      try {
        await customStatement('''
          ALTER TABLE user_study_steps 
          RENAME COLUMN index TO seq
        ''');
      } catch (e) {
        // 如果字段不存在或已经重命名，忽略错误
        // 可能的原因：字段已经迁移过，或者表结构已经是新版本
      ...
      // 1.3 重命名 learning_words 表的字段（大驼峰转小驼峰）
      // AddDay -> addDay, AddTime -> addTime, LastLearningDate -> lastLearningDate
      // LearningOrder -> learningOrder, LifeValue -> lifeValue
      try {
        await customStatement('''
          ALTER TABLE learning_words 
          RENAME COLUMN AddDay TO addDay
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE learning_words 
          RENAME COLUMN AddTime TO addTime
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE learning_words 
          RENAME COLUMN LastLearningDate TO lastLearningDate
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE learning_words 
          RENAME COLUMN LearningOrder TO learningOrder
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE learning_words 
          RENAME COLUMN LifeValue TO lifeValue
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      
      // 1.4 重命名 sentences 表的字段
      // English -> english, TheType -> theType, LastDiyUpdateTime -> lastDiyUpdateTime
      try {
        await customStatement('''
          ALTER TABLE sentences 
          RENAME COLUMN English TO english
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE sentences 
          RENAME COLUMN TheType TO theType
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE sentences 
          RENAME COLUMN LastDiyUpdateTime TO lastDiyUpdateTime
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      
      // 1.5 重命名 users 表的微信相关字段（下划线转驼峰）
      // wechat_open_id -> wechatOpenId, wechat_union_id -> wechatUnionId
      // wechat_nickname -> wechatNickname, wechat_avatar -> wechatAvatar
      try {
        await customStatement('''
          ALTER TABLE users 
          RENAME COLUMN wechat_open_id TO wechatOpenId
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE users 
          RENAME COLUMN wechat_union_id TO wechatUnionId
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE users 
          RENAME COLUMN wechat_nickname TO wechatNickname
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
      ...
      try {
        await customStatement('''
          ALTER TABLE users 
          RENAME COLUMN wechat_avatar TO wechatAvatar
        ''');
      } catch (e) {
        // 字段可能不存在或已重命名
      }
