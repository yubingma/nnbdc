    ...
    try {
      ...
      if (user != null) {
        try {
          ...
          Global.logger.i('getLoggedInUser 跨天检测：user.lastLearningDate=${user.lastLearningDate}, today=$today, todayStudyStarted=${user.todayStudyStarted}');
          if (user.todayStudyStarted && user.lastLearningDate != null &&
              user.lastLearningDate != today) {

             try {
               Global.logger.w('getLoggedInUser: 检测到持久过的今日状态已过期(跨天)，正在重置... lastLearningDate=${user.lastLearningDate}, today=$today');
