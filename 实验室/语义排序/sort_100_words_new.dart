...
void main() async {
  ...
  if (!wordsFile.existsSync()) {
    print('Error: test_100_words.jsonl not found');
    return;
  }
  ...
  // 1. 旧维度下的排序结果 (Scheme B, M=20)
  buildSignatures(words, old64);
  final List<String> pathOld = runSignatureFilteredTsp(words, 20);
  final List<String> pathOldOpt = optimizePath2OptLocal(pathOld, words, 15);
  final metricsOld = calculateMetrics(pathOldOpt, words);

  // 2. 新高熵低噪维度下的排序结果 (Scheme B, M=20)
  buildSignatures(words, new64);
  final List<String> pathNew = runSignatureFilteredTsp(words, 20);
  final List<String> pathNewOpt = optimizePath2OptLocal(pathNew, words, 15);
  final metricsNew = calculateMetrics(pathNewOpt, words);

  final buffer = StringBuffer();
  buffer.writeln('=== 100个单词新旧高熵维度排序效果比对 ===\n');

  buffer.writeln('【算法指标对比】');
  buffer.writeln('1. 旧高熵维度 (方差最大):');
  buffer.writeln('   - 平均相邻距离: ${metricsOld.avgDist.toStringAsFixed(2)}');
  buffer.writeln('   - 最大相邻距离: ${metricsOld.maxDist}');
  buffer.writeln('2. 新高熵低噪维度 (黄金维度):');
  buffer.writeln('   - 平均相邻距离: ${metricsNew.avgDist.toStringAsFixed(2)}');
  buffer.writeln('   - 最大相邻距离: ${metricsNew.maxDist}');
  buffer.writeln('--------------------------------------------------\n');

  buffer.writeln('【比对列 1】旧高熵维度 (方差最大, 含噪声) | 【比对列 2】新高熵低噪维度 (黄金特征)');
  buffer.writeln('-------------------------------------------+-------------------------------------------');

  final Map<String, WordRecord> map = {for (var w in words) w.id: w};

  for (int i = 0; i < words.length; i++) {
    final wOld = map[pathOldOpt[i]]!;
    final wNew = map[pathNewOpt[i]]!;

    String distOldStr = '';
    if (i > 0) {
      final prevWO = map[pathOldOpt[i - 1]]!;
      distOldStr = '(${computeHammingDistance(prevWO.emb32, wOld.emb32)})';
    } else {
      distOldStr = '(start)';
    }
    ...
    String distNewStr = '';
    if (i > 0) {
      final prevWN = map[pathNewOpt[i - 1]]!;
      distNewStr = '(${computeHammingDistance(prevWN.emb32, wNew.emb32)})';
    } else {
      distNewStr = '(start)';
    }

    final col1 = '${(i + 1).toString().padLeft(3, '0')}. ${wOld.spell.padRight(18)} $distOldStr'.padRight(42);
    final col2 = '${(i + 1).toString().padLeft(3, '0')}. ${wNew.spell.padRight(18)} $distNewStr';

    buffer.writeln('$col1 | $col2');
  }

  final outFile = File('实验室/语义排序/sorted_100_words_new_comparison.txt');
  outFile.writeAsStringSync(buffer.toString());
  print('✅ 结果已成功写入 实验室/语义排序/sorted_100_words_new_comparison.txt');
}
