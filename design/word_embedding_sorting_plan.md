...
- **生词本个性化整理**：用户的生词本通常来源杂乱，通过语义排序，可以将碎片化的生词整理成语义关联 of 词群，极大降低记忆复荷。

### 1.1 应用场景展示 (Supported Scenarios)

基于我们设计的**“后端保存 1024 维原始向量 + 前端同步 3D 降维坐标 + 支持 3D-TSP 路径重排”**架构，系统能够完美支持以下丰富的使用场景：
...
* **技术实现**：
  1. 后端接收用户输入的自然语言查询，调用 Embedding API 将其转化为 1024 维查询向量 $\vec{q}$。
...
* **技术实现**：直接在本地或后端查询与当前词 3D 坐标或 1024D 向量距离最近的 Top 5 单词。

#### 场景五：3D 语义星空可视化宇宙（3D Starfield）
* **用户体验**：提供一个酷炫的 3D 星空界面。用户背过的每个单词都化为一颗繁星。**“科技词汇”**汇聚成一片蓝色星云，**“情感词汇”**汇聚成一片粉色星云。用户可以旋转缩放星空，甚至直接点击某片星云“聚拢”复习。
...
* **技术实现**：后端通过 1024D 向量跑 K-Means 聚类，并将同一聚类簇的词分配到同一个 `unit` 编号中。
...
#### 2.1.2 高维（后端1024D）与低维（前端3D）职责边界分工 (Division of Responsibilities)

为了在“计算性能”、“网络开销”与“体验功能”之间取得最佳平衡，我们将高维向量与低维坐标在不同场景下的分工划分为以下两条明确的职责边界：

##### 1. 必须依赖后端高维向量（1024D）的场景
这类场景的特点是**“数据量庞大”**、**“需要精细无损的语义度量”**或**“涉及动态自然语言输入”**：
* **语义智能搜索**：用户输入任意查询词或句子（如 *“下雨天出行的装备”*），必须通过大模型编码为高维向量 $\vec{q}$。因为在 3 维极低维空间中无法直接映射复杂的未注册查询文本，必须在高维空间（1024D）中对全库单词进行无损的相似度检索（如 $\cos$ 夹角计算）。
* **全局高保真同义词/近义词挖掘**：当要在几万个单词的庞大词库中寻找绝对精准的同义词时，降维带来的信息损耗可能会导致“假阳性”冲突（3D 距离近的词在高维中其实是两个主题）。全局、高精度的语义关联必须在 1024D 空间计算。
* **降维模型自身的重新训练（Fit）**：当引进大量新词或更换嵌入模型时，必须使用全库所有词的 1024D 原始向量作为输入，才能拟合出最新的 PCA 投影矩阵或 UMAP 降维模型。

##### 2. 仅依赖前端低维坐标（3D）即可做到的场景
这类场景的特点是**“在客户端离线运行”**、**“数据规模较小（如百词级别）”**或**“侧重视觉渲染”**：
* **3D 可视化语义星空渲染**：客户端仅需读取同步到的 `(vecX, vecY, vecZ)`，即可借助 Flutter 3D 渲染引擎快速绘制出绚丽的 3D 星云图，完全不需要也不可能在前端直接渲染 1024 维数据。
* **用户生词本（百词级别）的本地一键重排（3D-TSP）**：用户想将自己生词本里的 100 个单词按照语义连贯排序。由于数据量小（仅 100 词），在 3 维坐标下计算欧氏距离极其快速，手机端可以在 1 毫秒内完全离线完成 TSP 排序。而如果使用 1024D 向量，手机端的计算耗时和资源消耗会大幅增加。
* **生词本本地快速分单元（3D-KMeans）**：在手机本地离线把用户的 150 个生词自动切分成 5 个语义相关的 Unit。在 3 维空间运行 KMeans 算法非常轻量，体验极其流畅。
* **背单词时的邻近词微弱星光连线提示**：在本地背单词过程中，快速提示与当前词 3D 空间距离最近的下一个生词。
